...
contract Crowdsale is Owned {
	...
	function setDurationUnits(uint _durationUnits) onlyOwner public {
		durationUnits = _durationUnits;
	}

	function getDurationUnits() constant onlyOwner public returns (uint) {
	...
	function currentSwapRate() constant public returns(uint) {
		if(startTime + 3 * durationUnits > now) {
			return 100;
		} else if(startTime + 5 * durationUnits > now) {
			return 50;
		} else if(startTime + 10 * durationUnits > now) {
			return 20;
		} else {
			return 0;
		}
	...
	function getBaseInfo() constant public returns (uint, uint, uint, address, uint, address) {
		return (fundingGoal, price, transferableToken, address(tokenReward), startTime, owner);
	}

	function getFundingInfo() constant public returns (bool, bool, uint) {
		return (isOpened, fundingGoalReached, soldToken);
	}
